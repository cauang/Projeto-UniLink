...
import { Calendar as CalendarIcon, X, ChevronLeft, ChevronRight, User, Filter } from 'lucide-react';
...
const THEME = {
  ...
  pendingText: '#854D0E',    // Texto amarelo escuro
  confirmedBg: '#DCFCE7',    // Verde claro
  confirmedText: '#166534',  // Texto verde escuro
  availableBg: '#DBEAFE',    // Azul claro (Disponíveis)
  availableText: '#1E40FF',  // Azul escuro (Disponíveis)
  todayBg: '#F3F4F6',        // Cinza claro para hoje
};
...
// Função auxiliar para checar se é "Disponível"
const isAvailableEvent = (ev) => {
  const status = (ev.status || '').toLowerCase();
  const type = (ev.tipo || '').toLowerCase();
  return status === 'disponivel' || type === 'disponivel';
};

export default function CalendarWidget({ visible, onClose, events = [] }) {
  const [current, setCurrent] = useState(() => new Date());
  const [selected, setSelected] = useState(null);
  const [filter, setFilter] = useState('todos'); // 'todos', 'meus', 'disponiveis'
  ...
  const filteredEventsList = useMemo(() => {
    ...
    return events.filter(ev => {
      const isAvailable = isAvailableEvent(ev);
      if (filter === 'disponiveis') return isAvailable;
      if (filter === 'meus') return !isAvailable; // "Meus" são os que NÃO são "disponíveis"
      return true;
    });
  }, [events, filter]);

  // 2. Agrupamento por data (usando a lista já filtrada)
  ...
  // 3. Resumo (Baseado no mês atual e no filtro atual)
  const summary = useMemo(() => {
    ...
    return { total, confirmados, pendentes, disponiveis };
  }, [filteredEventsList, current]);
  ...
  // Auxiliar para pegar estilo da pílula
  const getEventStyle = (ev) => {
    if (isAvailableEvent(ev)) {
      return { bg: THEME.availableBg, text: THEME.availableText, label: 'Disponível', solidBg: '#3B82F6' }; // Azul
    }
    const status = (ev.status || '').toLowerCase();
    if (status.includes('pendente')) {
      return { bg: THEME.pendingBg, text: THEME.pendingText, label: 'Pendente', solidBg: '#EAB308' }; // Amarelo
    }
    return { bg: THEME.confirmedBg, text: THEME.confirmedText, label: 'Confirmado', solidBg: THEME.primary }; // Verde
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/30">
      ...
      >
        ...
        {/* --- HEADER VERDE --- */}
        <div 
          className="text-white p-6 flex flex-col md:flex-row items-start md:items-center justify-between shrink-0 gap-4"
          style={{ backgroundColor: THEME.primary }}
        >
          <div>
            <h3 className="text-2xl font-bold">Calendário de Procedimentos</h3>
            <p className="text-sm opacity-90 mt-1">Visualize todos os seus agendamentos e oportunidades</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-end md:items-center gap-6 w-full md:w-auto">
            ...
            <div className="flex bg-black/10 p-1 rounded-lg">
                <button 
                    ...
                    className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${filter === 'todos' ? 'bg-white text-[#00A63E] shadow-sm' : 'text-white hover:bg-white/10'}`}
                >
                    Todos
                </button>
                <button 
                    onClick={() => setFilter('meus')}
                    className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${filter === 'meus' ? 'bg-white text-[#00A63E] shadow-sm' : 'text-white hover:bg-white/10'}`}
                >
                    Inscritos
                </button>
                <button 
                    onClick={() => setFilter('disponiveis')}
                    className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${filter === 'disponiveis' ? 'bg-white text-[#00A63E] shadow-sm' : 'text-white hover:bg-white/10'}`}
                >
                    Disponíveis
                </button>
            </div>
                          ...
                          <div className="flex flex-col gap-1 mt-1">
                            {dayEvents.slice(0, 2).map((ev, idx) => {
                                ...
                                return (
                                    ...
                                    style={{ backgroundColor: styles.solidBg }}
                                    >
            ...
            <div className="space-y-6">
                        ...
                        <div className="h-full">
                            {(eventsByDate[selectedKey] || []).length === 0 ? (
                                <div className="text-center text-gray-400 flex flex-col items-center justify-center h-full gap-3">
                                    ...
                                    <p>Nenhum procedimento para este filtro neste dia.</p>
                                </div>
                            ) : (
                                <div className="space-y-3 overflow-y-auto max-h-[250px] pr-2 custom-scrollbar">
                                    {(eventsByDate[selectedKey] || []).map((ev, idx) => {
                                        const styles = getEventStyle(ev);
                                        return (
                                            <div key={idx} className="p-3 rounded-lg border border-l-4 border-gray-100 hover:shadow-md transition-shadow" style={{ borderLeftColor: styles.solidBg }}>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                ...
                <div className="space-y-3">
                    ...
                    {/* Mostra os contadores dependendo do filtro ativo */}
                    {(filter === 'todos' || filter === 'meus') && (
                      <>
                        <div className="flex items-center justify-between">
                            <span className="text-gray-600">Confirmados</span>
                            <span className="px-3 py-1 rounded-md font-bold text-sm" style={{ backgroundColor: THEME.confirmedBg, color: THEME.confirmedText }}>
                                {summary.confirmados}
                            </span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-gray-600">Pendentes</span>
                            <span className="px-3 py-1 rounded-md font-bold text-sm" style={{ backgroundColor: THEME.pendingBg, color: THEME.pendingText }}>
                                {summary.pendentes}
                            </span>
                        </div>
                      </>
                    ...
                    {(filter === 'todos' || filter === 'disponiveis') && (
                      <div className="flex items-center justify-between">
                          <span className="text-gray-600">Disponíveis</span>
                          <span className="px-3 py-1 rounded-md font-bold text-sm" style={{ backgroundColor: THEME.availableBg, color: THEME.availableText }}>
                              {summary.disponiveis}
                          </span>
                      </div>
                    )}
